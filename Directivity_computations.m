...
%  geometry = geometry_open_square; %%%% open resonator
%  N_angles = 72; %%% number of independent solutions for embedding
...
%%
 figure;
 drawnow
 absD = max(abs(Directivities(:)));

 for n_cur = 1: size(Directivities,1)
     plot(theta_ar_obs,abs(Directivities(n_cur,:)))
      axis([0 pi/2 0 absD])
     title(theta_ar_in(n_cur))
     pause(0.1)     
 end
