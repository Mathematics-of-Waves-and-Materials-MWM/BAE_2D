...
%%%%%%% sides of the boundary
   ...
   for n_cur = 1: size(body_boundary,1)-1
       loc_mask_squares = true(num_squares,1);
       for m_cur = 0:1
        ...
        loc_mask_squares = loc_mask_squares&connectivity_dist<0.5;
        cur_indices = find(loc_mask_squares);
       end
...
cur_mask_boundary = [cur_mask_boundary, true(1,num_add_nodes)];
cur_mask_outer_nodes = [cur_mask_outer_nodes; true(num_add_nodes,1)];
cur_mask_adjacent_boundary = [cur_mask_adjacent_boundary;num_nodes + 1:num_add_nodes];
...
%%%%% adjust one set of adjacent connectivities
  ...
  mask_boundary = [mask_boundary,false(1,num_add_nodes)];
  mask_inner_nodes = [mask_inner_nodes, false(1,num_add_nodes)];
  cur_mask_inner_nodes = [cur_mask_inner_nodes, false(1,num_add_nodes)];
   % figure;
